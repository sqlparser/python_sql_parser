...
def call_dataFlowAnalyzer(args):
    ...
    jar = "-Djava.class.path=D:\\work\gudu\\gsp\\gsp_java_core\\target\\gudusoft.gsqlparser-2.8.5.4.jar"
    ...
    try:
        ...
        TSQLEnv = jpype.JClass("gudusoft.gsqlparser.sqlenv.TSQLEnv")
        ...
        if indexOf(args, "/f") != -1 and len(args) > indexOf(args, "/f") + 1:
           ...
           if not sqlFiles.exists() or not sqlFiles.isDirectory():
              ...
              return
        elif indexOf(args, "/fromdb") != -1 and len(args) > indexOf(args, "/fromdb") + 1:
           metadataoutput = "metadata.json"
           if indexOf(args, "/metadataoutput") != -1 and len(args) > indexOf(args, "/metadataoutput") + 1:
             metadataoutput = args[indexOf(args, "/metadataoutput") + 1]
           SqlflowIngester = jpype.JClass("gudusoft.gsqlparser.dlineage.SqlflowIngester")
           if not SqlflowIngester.export(vendor.name(), args[indexOf(args, "/fromdb") + 1].split("\\s+"), metadataoutput):
             return
           if indexOf(args, "/exportonly") != -1:
             return
           sqlFiles = File(metadataoutput)
        else:
        ...
        if dataflow != None and indexOf(args, "/graph") != -1:
            ...
            save_to_file("lineageGraph.json", str(result))
            cmd = "SQLFlowClient -file lineageGraph.json"
            os.system(cmd)
...
if __name__ == "__main__":
    ...
    if len(args) < 2:
      ...
      print("/log: Optional, generate a dataflow.log file to log information.")
      ...
      print("/fromdb: Optional, specifies the database connection parameters.")
      print("/exportonly: Optional, just export metadata.json, no further data analysis.")
      print("/metadataoutput: Optional, specifies the metadata output directory and file name.")
